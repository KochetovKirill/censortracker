...
- **The README no longer promises a `.xpi` that releases do not carry.** Signing
  needs AMO credentials this repository does not have, so the release workflow
  skips it and ships the unsigned Firefox `.zip` — which stable Firefox refuses
  as "corrupt", leaving the documented install path leading nowhere. The
  installation sections now say which asset a release actually has, and give the
  two ways an unsigned build can be installed (`about:debugging` temporarily, or
  Developer Edition / Nightly / ESR with `xpinstall.signatures.required` off).
  The release notes generated by CI state the same thing per release, and the
  signing step now also reports a failure when `web-ext sign` exits cleanly
